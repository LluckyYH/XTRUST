...
ChatGLM_1_path = r'/home/liyahan/models/chatglm-6b'
...
class ChatGLM_1_Evaluator(Evaluator):
    def __init__(self, choices, k, model_name, t, s, pt, es,l, device):
        super(ChatGLM_1_Evaluator, self).__init__(choices, model_name, k)
        self.pt = pt
        self.es = es
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(ChatGLM_1_path, trust_remote_code=True)
        # self.model = AutoModel.from_pretrained(ChatGLM_1_path,torch_dtype=torch.bfloat16, trust_remote_code=True).to(device)
        self.model = AutoModel.from_pretrained(ChatGLM_1_path, trust_remote_code=True, mirror="tuna",
