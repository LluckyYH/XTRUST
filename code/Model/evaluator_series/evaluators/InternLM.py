...
InternLM_path = '/home/liyahan/models/internlm-chat-7b'
...
def get_prompt(line=None, subject_name="", task="", scenario="", pt=0,es=0, language=""):
    ...
    return prompt
class InternLM_Evaluator(Evaluator):
    def __init__(self, choices, k, model_name, t, s, pt, es,l, device):
        super(InternLM_Evaluator, self).__init__(choices, model_name, k)
        self.pt = pt
        self.es = es
        ...
        self.scenario = s
        self.language = l
        self.tokenizer = AutoTokenizer.from_pretrained(InternLM_path, trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained(InternLM_path, trust_remote_code=True, mirror="tuna",resume_download = True).half().to(device)
    ...
    def eval_MultiLanguage(
        ...
        if save_result_dir:
            ...
            test_df.to_csv(
                os.path.join(
                    save_result_dir, f"{subject_name}_{self.scenario}__{file}{pii}_test.csv"
                )
            )
