...
ChatGLM_3_path = r'/home/liyahan/models/chatglm3-6b'
...
def get_prompt(line=None, subject_name="", task="", scenario="", pt=0,es=0, language=""):
    ...
    return prompt
class ChatGLM_3_Evaluator(Evaluator):
    def __init__(self, choices, k, model_name, t, s, pt, es,l, device):
        super(ChatGLM_3_Evaluator, self).__init__(choices, model_name, k)
        self.pt = pt
        self.es = es
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(ChatGLM_3_path, trust_remote_code=True)
        self.model = AutoModel.from_pretrained(ChatGLM_3_path,torch_dtype=torch.bfloat16, trust_remote_code=True).to(device)
    ...
    def generate_few_shot_prompt(self, subject_name, dev_df, cot=False):
        ...
        if self.k == -1:
            k = dev_df.shape[0]
        message.append(
            self.history_format_example(dev_df.iloc[0, :], cot=cot, subject_name=subject_name)
        )
        for i in range(1, k):
            message.append(
                self.history_format_example(
